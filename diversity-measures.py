...
def matrix_generator(predict1,
    ...
    N10 = 0
    N11 = 0
    if (len(predict1) != len(correct_answer) or
        len(predict2) != len(correct_answer)):
        raise ValueError(
    ...
    for instance in range(len(predict1)):
        if predict1[instance] == predict2[instance]:
            ...
            if predict1[instance] == correct_answer[instance]: 
                ...
                if predict2[instance] == correct_answer[instance]:
                    N01 += 1 
    return N00,N01,N10,N11


def pw_double_fault_measure(predict1,
                            predict2,
                            correct_answer):
    N00, _, _, _ = matrix_generator(predict1,
                                    predict2,
                                    correct_answer,
                                    visualize=False)
...
def pw_q_statistic(predict1,
                   predict2,
                   correct_answer): #n11*n00 - n01*n10 / n11*n00 + n01*n10
    N00, N01, N10, N11 = matrix_generator(predict1,
                                          predict2,
                                          correct_answer)
...
def disagreement(predict1,
                 predict2,
                 correct_answer):
            
    _, N01, N10, _ = matrix_generator(predict1,
                                      predict2,
                                      correct_answer)
    return (N01 + N10) /
            len(predict1)
...
def pw_agreement(predict1,predict2,correct_answer):
    ...
    N00, _, _, N11 = matrix_generator(predict1,
                                      predict2,
                                      correct_answer)
    return (N11 + N00) /
            len(predict1)
...
def avg_disagreement(predicts_vector,
                     correct_answer):
    ...
    for clf1 in range(clf1,len(predicts_vector)):
        for clf2 in range(clf1+1,len(predicts_vector)):
            disagreement_sum += disagreement(predicts_vector[clf1],
                                             predicts_vector[clf2],
                                             correct_answer)
...
def kohavi_wolpert_variance(n_class,
                            avg_disagreement):
    return (((n_class-1)* 
             avg_disagreement) /
            (2*n_class))

def entropy_measure(predicts_vector,
                    correct_answer):
    corrects = 0
    ...
    for instance in range(len(predicts_vector[0])):
        for classifier in range(n_classifiers):
            if predicts_vector[classifier][instance] == correct_answer[instance]:
                corrects +=1
        less_frequent = min(corrects,
                            n_classifiers-corrects)
